 ...
 * Patterns (2.1.7):
 *
 * AchievementGui::updateModdedLabel (was AchievementGui::refresh) JZ > JNZ -- XREF gui-achievements.modded-game
 * near-JE (0F 84), 0F prefix discarded; ? = displacement byte (changes between versions)
 * 84 ? 00 00 00 48 8b 10 80 7a 3e 01 75 ea
 *
 * note for me: this is the achievements.dat & achievements-modded.dat thingy
 * PlayerData::PlayerData JZ > JMP (was JZ > JNZ)
 * 74 e2 48 89 d3 48 b8 ff ff ff ff ff ff ff 7f
 *
 ...
 *
 * SteamContext::updateAchievementStatsFromSteam JZ > JMP
 * 74 ?  48 8b ?  80 7A ?  ?  ?  ?  80 7A ?  ?  ?  ?  80 7A ?  ?  ?  ?  e9 22 01 00 00
 *
 * SteamContext::unlockAchievementsThatAreOnSteamButArentActivatedLocally JZ > JMP
 * anchor: 48 ba 74 65 73 74 5f 6d 6f 64 = movabs rdx,"test_mod"; ? = variable displacement/NOP bytes
 * 74 ? 48 ba 74 65 73 74 5f 6d 6f 64 eb ? 0f 1f 84 00 00 00 00 00 48 83 c0 08
 *
 * AchievementGui::allowed (map) jz > jmp
 * 74 07 48 83 78 20 00 75 cc   JZ > JNZ
...
std::vector<patternData_t> patternList = {
    /*
        JZ -> JNZ Patches
    */
    {PATCH_TYPE_JZJNZ, "AchievementGui::updateModdedLabel",
    "84 ? 00 00 00 48 8b 10 80 7a 3e 01 75 ea"},
